...
const updateServer = async ({ id, host, port, api_key, owner_id, user }) => {
    ...
    if (subscription?.toLowerCase() === 'premium') {
        ...
        if (info) await updateInTable(tables.linuxServers, [
...
module.exports = {
    execute: async () => {
        const { data: { rows: servers } } = await selectInTable(tables.linuxServers, 'id,monitoring,owner_id,host,port,api_key');
